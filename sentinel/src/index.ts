...
async function main() {
  if (!fs.existsSync(SENTINEL_KEYPAIR_PATH)) {
    console.error(`[sentinel] Keypair not found at ${SENTINEL_KEYPAIR_PATH}`);
    console.error(`[sentinel] Run: solana-keygen new --outfile ${SENTINEL_KEYPAIR_PATH}`);
    process.exit(1);
  }
  ...
  console.log(`[sentinel] Polling:   ${THREAT_API_URL}/status every ${POLL_INTERVAL_MS}ms`);
...
async function poll(provider: anchor.AnchorProvider, keypair: Keypair) {
  pollCount++;
  const res = await axios.get<{ level: ThreatLevel; timestamp: string }>(`${THREAT_API_URL}/status`);
  const { level, timestamp } = res.data;

  if (level !== lastKnownLevel) {
    console.log(`[sentinel] #${pollCount} ${timestamp} | Level changed: ${lastKnownLevel} → ${level}`);
    lastKnownLevel = level;
  } else {
    console.log(`[sentinel] #${pollCount} ${timestamp} | Level: ${level}`);
  }

  if (level === "critical" && !lockdownFired) {
    console.log(`[sentinel] ⚠️  CRITICAL threat — firing migrate instruction...`);
  ...
  if (level !== "critical" && lockdownFired) {
    lockdownFired = false;
    console.log(`[sentinel] Threat cleared — ready to fire again if critical returns`);
  }
}
